...
class userValidator {
    ...
    validateUserSignUp = () => {
        ...
        return [
            ...
            body('password').notEmpty().withMessage('Envia una contraseña').isLength({ min: 8 }).withMessage('La contraseña debe tener al menos 8 caracteres').isString().withMessage('La contraseña debe ser una cadena de texto'),
            ...
            body().custom((_, { req }) => {
                const allowedFields = ['identifier', 'nickName', 'password', 'email', 'profilePicture', 'address', 'phone', 'type'];
